...
Скриншот переключения на резервный сервер при не доступности 80 порта
![Переключение на резервный сервер при падении веб-сервера](/img/Screenshot_switch_no_http.png)

Скриншот переключения на резервный сервер при отсутствии страницы
...
Скриншот переключения при работе 3-х серверов.
Приоритет определяется скриптом.
Алгоритм использует 2-е значение (за 5 минут) Load Average, как более плавное изменение по сравнению с первым значением (за 1 минуту). Такое решение выбрано как пример. Так же учитывается количество процессоров, т.к. на других серверах может быть другое количество процессоров, поэтому абсолютное значение Load Average нельзя сравнивать без учета количества процессоров.
